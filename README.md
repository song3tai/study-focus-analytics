...
- 后续可演进为基于 Electron 的 Windows 桌面应用
- Electron 主要承担桌面容器与产品化包装职责，不承担 AI 推理逻辑
...
- V1.5：在此基础上增加 Electron 桌面壳
- 当前不以云端集中部署为主要目标

这种推进方式更适合当前阶段：先验证行为分析链路、专注度估计和产品交互是否真正有价值，再考虑进一步的安装体验与桌面化封装。
...
- 增加 Electron 桌面壳
- 在 Windows 上作为本地桌面应用运行
- 由 Electron 承载本地 React UI，并与本地 Python 分析服务配合
- 优化状态分析稳定性与桌面端体验

### V2

- 优化桌面应用体验
- 增加本地配置、历史分析结果管理、离线分析结果页等能力
- 继续增强行为识别与专注度估计质量
