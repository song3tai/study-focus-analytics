...
本文档用于说明 Study Focus Analytics 的系统设计、模块边界、核心数据流和当前阶段的架构取舍，面向开发者、未来维护者以及需要理解系统实现方式的人。
...
- 运行形态以本地优先（Local-first）为主，优先在单机环境完成分析与展示
...
因此，当前阶段的核心任务已经从“构建分析链路”转向“打造可用的分析工具”。
...
当前项目更适合按 Local-first 架构推进，而不是在 V1 直接做云端集中分析平台。

原因包括：

- 云端视频分析会引入较高的 GPU 成本
- 视频上传与结果回传会带来持续的带宽与流量成本
- 当前 MVP 阶段的重点是验证行为分析能力和产品价值，而不是投入复杂云部署
- 对于个人学习 / 工作状态分析场景，本地运行更符合隐私友好和低成本原则

因此，当前推荐的默认假设是：

- 分析后端运行在用户本机
- 前端界面也运行在用户本机
- 模型、视频输入和结果输出优先在本地闭环完成
...
在 V1 主链路已经完成的基础上，V1.5 的重点不再是继续搭建分析骨架，而是把现有能力组织成可用的离线分析体验。
...
- 让分析结束后直接形成结果输出
- 让用户可以查看汇总、时间线和事件流
- 让系统更像“复盘工具”，而不只是“过程演示系统”

V1.5 的关键判断标准不是“实时画面是否更丰富”，而是“分析结果是否更可消费”。
...
这些内容并非永远不会做，而是它们会显著增加系统复杂度，并稀释当前最核心的问题：如何在单人固定场景下构建一条真实可跑、可解释、可展示、可复盘的行为分析链路。

## 总体架构

系统推荐采用如下总体架构流：

`Video Source -> Input Layer -> Detection Layer -> Scene Feature Layer -> State Tracker -> Focus Estimator -> Event / Summary Aggregator -> Result Builder -> FastAPI / WebSocket -> React Dashboard`

同时，当前推荐的部署架构流为：

```text
Windows 本地环境
├── Python 分析服务（FastAPI + WebSocket）
├── React 前端
└── 本地视频输入 / 本地模型 / 本地结果输出
```
...
后端的核心不是“把视频显示出来”，而是把连续帧转换成稳定的行为语义，并最终组织成可消费的结果对象。
...
前端不负责行为判断逻辑，不复制状态机，不重新推导专注度。它消费的是后端已经生成的结构化结果和结果系统输出。
...
- `fast` 模式优先于 `realtime`
- 离线结果优先于实时画面
- 视频文件输入默认应逐步朝“快速分析优先”演进
- 摄像头和 RTSP 仍属于实时分析场景，不强行复用 fast 语义

## 分析结果输出模型

系统最终输出不应只停留在逐帧 `ProcessResult`，而应形成完整结果系统。

### 1. Summary

汇总结果，用于快速回答一段视频的核心结论，例如：
...
V1 当前不以“视频上传到云端，再由云端 GPU 做实时推理”的模式为目标。

主要原因包括：

- 成本高：实时视频分析会显著放大 GPU 成本
- 带宽高：视频上传和结果回传会持续消耗网络资源
- 复杂度高：云端推理、任务调度、存储和部署链路都会明显增加工程复杂度
- 当前重点不在云部署：现阶段最重要的是让产品在本机可跑、可演示、可安装

因此，V1 更适合先做出一套本地可运行的分析工具，再逐步推进桌面化封装和产品体验。
...
- `ProcessResult` 是中间结果
- 它主要服务于处理链路、调试、实时界面和过程可视化
- 它不是用户最终消费的核心对象

### Session Result

在 `ProcessResult` 之上，系统应继续组织会话级结果对象，作为离线分析模式的最终输出。
...
换句话说，图像渲染是附属能力，结构化结果才是系统核心产物；而在当前阶段，结果系统又高于单帧结构化结果。
...
在当前阶段，API 不应只服务于“实时过程查看”，还应逐步服务于“离线结果查询”。
...
- `GET /health`：健康检查
- `GET /analysis/status`：获取当前分析服务状态与会话状态
- `GET /analysis/latest`：获取最近一次结构化分析结果
- `GET /analysis/summary`：获取当前内存中的最新汇总统计
- `POST /analysis/start`：启动一次本地分析会话
- `POST /analysis/stop`：停止当前分析会话
- `WS /ws/analysis`：推送服务状态和最新分析结果快照
...
在 V1.5 阶段，建议继续向“结果系统接口”演进。

### 后续建议的 REST API

在当前基础上，后续如果需要扩展更细的展示或历史查询能力，建议优先沿用现有 `/analysis/*` 路径风格继续演进，例如：

- `GET /analysis/timeline`
- `GET /analysis/events`
- `GET /analysis/history`
- `GET /analysis/session-result`

职责建议如下：

- `GET /analysis/timeline`：获取前端绘制时间线所需的数据
- `GET /analysis/events`：获取结构化事件列表
- `GET /analysis/history`：获取历史分析记录或离线分析结果索引
- `GET /analysis/session-result`：获取一次分析完成后的完整结果对象

### 后续建议的 WebSocket API

当前实现中 WebSocket 使用的是 `WS /ws/analysis`。后续如需拆分实时通道，建议优先在现有命名体系上演进，例如：

- `WS /ws/analysis/events`
- `WS /ws/analysis/status`
...
以下内容用于前后端联调和接口实现对齐，默认基于当前实际代码实现。
...
- 查询当前分析服务状态
- 查询当前会话是否正在运行
- 查询最近一次处理帧的索引、时间戳和错误状态

响应字段：

- `running`：是否有分析会话正在运行
- `session_state`：当前会话状态，常见值包括 `idle`、`starting`、`running`、`stopping`、`error`
- `source_type`：当前输入源类型，可能为 `camera`、`video_file`、`rtsp`
- `source`：当前输入源标识
- `started_at`：会话启动时间，UTC ISO-8601 格式
- `has_latest_result`：是否已有最近结果
- `last_frame_id`：最近处理帧编号
- `last_timestamp`：最近处理结果对应的时间戳
- `last_error`：最近错误信息
...
```json
{
  ...
  "started_at": "2026-03-24T03:00:00.000000+00:00",
...
- 获取最近一次结构化分析结果
- 供 Dashboard 拉取当前帧对应的完整分析快照

响应字段：

- `has_result`：是否已有结果
- `data`：最近一次 `ProcessResult` 的序列化结果
- `message`：附加说明
...
```json
{
  ...
  "data": {
    ...
    "state_snapshot": {
      ...
      "state_duration_sec": 1.2,
      "away_count": 0
    },
    "focus_estimate": {
      "focus_score": 0.63,
      "focus_level": "medium",
      "reasons": ["state=present", "in_study_roi", "low_motion"]
    },
    "summary": {
      "total_duration_sec": 1.4,
      "total_present_duration_sec": 1.2,
      "total_away_duration_sec": 0.0,
      "total_studying_duration_sec": 0.0,
      "away_count": 0,
      "average_focus_score": 0.58,
      "max_focus_score": 0.72,
      "min_focus_score": 0.31,
      "focus_samples": 42
    }
  },
...
- 示例中的 `data` 为节选，实际返回字段以 `ProcessResult.to_dict()` 输出为准
...
- 获取当前内存中的最新汇总统计
- 供 Dashboard 的摘要卡片直接使用

响应字段：

- `has_summary`：是否已有汇总
- `data`：最新 `AnalysisSummary` 序列化结果
- `message`：附加说明
...
```json
{
  ...
  "data": {
    ...
    "total_present_duration_sec": 95.2,
    "total_away_duration_sec": 25.3,
    "total_studying_duration_sec": 60.4,
    "away_count": 2,
    "average_focus_score": 0.62,
    "max_focus_score": 0.88,
    "min_focus_score": 0.1,
    "focus_samples": 3615
  },
...
- `source_type`：必填，取值为 `camera`、`video_file`、`rtsp`
- `source`：可选；当 `source_type` 为 `video_file` 或 `rtsp` 时必填；`camera` 模式下可省略，默认使用配置中的摄像头索引
- `debug`：可选，是否启用调试日志
...
{
  "success": true,
  "message": "analysis session starting",
...
{
  "success": false,
  "message": "analysis session is already active",
  "session_state": "running"
}
```
...
{
  "type": "service_status",
  "timestamp": "2026-03-24T03:00:00.000000+00:00",
  "data": {
    "running": true,
    "session_state": "running",
    "source_type": "camera",
...
{
  "type": "process_result",
  "timestamp": "2026-03-24T03:00:01.000000+00:00",
  "data": {
    "frame_packet": {
      "frame_id": 12,
      "timestamp": 0.4
...
{
  "type": "service_error",
  "timestamp": "2026-03-24T03:00:05.000000+00:00",
  "data": {
    "message": "video file not found: input/missing.mp4",
    "session_state": "error"
  }
}
```
...
- 本地 React 前端可以直接通过 HTTP / WebSocket 访问本地 Python 服务
- 即使后续引入 Electron，也不等于必须增加一层 Node.js 分析后端
- 在本地架构下再引入 Node.js BFF，通常只会增加进程数、调试成本和维护复杂度
...
- 当前阶段不做 Electron
- Python 分析后端仍然是核心分析引擎
- React 仍然是主要展示层
- 不要把未来 Electron 误当作 AI 推理层

在这些核心问题尚未稳定之前，引入 BFF 不会解决主要矛盾，反而会转移注意力。

### V1.5

在 V1 主链路完成之后，V1.5 的目标是把系统从“可运行分析系统”推进到“可用的离线分析工具”。
...
Study Focus Analytics 在当前阶段应坚持以下设计原则：
...
如果用一句话概括当前阶段的设计取向，那就是：在既有分析主链路基础上，优先把“快速分析、汇总结果、事件时间线、离线结果页”这条结果闭环做扎实，再讨论更复杂的能力扩展。
