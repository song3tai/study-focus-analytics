...
当前项目已经具备一套旧版 `video_frame_processor` 结构，V1 的实现方式是在现有能力基础上逐步重构和迁移，而不是完全重写一套新系统。

建议的映射关系如下：

- `src/video_reader.py` -> `src/io/video_reader.py`
- `src/video_writer.py` -> `src/io/video_writer.py`
- `src/ai_detector.py` -> `src/inference/ai_detector.py`
- `src/frame_processor.py` -> V1 中逐步弱化，原有通用帧处理职责后续根据需要迁移到 `behavior` 层或被更明确的分析模块替代
- `src/pipeline.py` -> `src/pipeline/analysis_pipeline.py`
...
- `GET /api/summary`：获取当前会话或最近分析任务的汇总结果
